...
class HnTunnelVpnService : VpnService() {
    ...
    repeat(3) { attempt ->
      try {
        ...
        Socket().use { socket ->
          ...
          output.write(byteArrayOf(5.toByte(), 1.toByte(), 0.toByte(), 1.toByte(), 1.toByte(), 1.toByte(), 1.toByte(), 1.toByte(), 0.toByte(), 443.toByte()))
          output.flush()
          ...
          if (reply[0].toInt() == 5 && reply[1].toInt() == 0) {
            LogManager.s("Connectivity check: SOCKS -> 1.1.1.1:443 OK")
  ...
  private suspend fun connectXrayBackend(config: TunnelConfig, tunFd: Int) = withContext(Dispatchers.IO) {
    TunnelController.updateState(TunnelState.AUTHENTICATING)
    XrayCoreBridge.start(XrayConfigBuilder.build(config, LOCAL_SOCKS_PORT), tunFd, filesDir.absolutePath)
    if (!XrayCoreBridge.isRunning()) throw IllegalStateException("Xray tidak running")
  }
  ...
  private fun startStatsMonitor(config: TunnelConfig) {
    ...
    statsJob = serviceScope.launch {
      ...
      while (isActive && TunnelController.tunnelState.value == TunnelState.CONNECTED) {
        ...
        TunnelController.updateStats(TunnelStats(bytesIn = rx, bytesOut = tx, speedInBps = (rx - lastRx).coerceAtLeast(0) * 8, speedOutBps = (tx - lastTx).coerceAtLeast(0) * 8, durationSeconds = seconds))
        lastRx = rx
        lastTx = tx
      }
    }
  ...
  private fun stopTunnel() {
    serviceScope.launch {
      ...
      try { XrayCoreBridge.stop() } catch (_: Exception) {}
  ...
  private fun createNotificationChannel() {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
      getSystemService(NotificationManager::class.java)?.createNotificationChannel(NotificationChannel(CHANNEL_ID, "HN Tunnel Service", NotificationManager.IMPORTANCE_LOW))
    }
  }
  ...
  private fun buildNotification(title: String, content: String): Notification {
    ...
    return NotificationCompat.Builder(this, CHANNEL_ID).setSmallIcon(R.drawable.ic_launcher_foreground).setContentTitle(title).setContentText(content).setContentIntent(open).addAction(android.R.drawable.ic_menu_close_clear_cancel, "Disconnect", stop).setOngoing(true).setPriority(NotificationCompat.PRIORITY_LOW).build()
  }
